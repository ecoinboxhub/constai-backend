...
MLFLOW_EXPERIMENT = "nigeria_construction_delay"
...
def train(X: pd.DataFrame, y: pd.Series):
    ...
    rf_model, rf_params = try_optuna_classifier(X_train, y_train, X_test, y_test, "RandomForest")
    ...
    gb_model, gb_params = try_optuna_classifier(X_train, y_train, X_test, y_test, "GradientBoosting")
    ...
    if xgb_model is not None:
        ...
        xgb_tuned_model, xgb_tuned_params = try_optuna_classifier(X_train, y_train, X_test, y_test, "XGBoost")
        xgb_tuned_pred = xgb_tuned_model.predict(X_test)
        xgb_tuned_proba = xgb_tuned_model.predict_proba(X_test)[:, 1]
        xgb_tuned_metrics = evaluate_classifier(y_test, xgb_tuned_pred, xgb_tuned_proba)
